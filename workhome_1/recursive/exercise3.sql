...
with recursive cte_recommend as(select fst.memid as member,
					   fst.recommendedby as recommender,
					   fst.firstname, fst.surname from cd.members fst
					   union all
					   select o.member,
					   scd.recommendedby as recommender,
					   scd.firstname, scd.surname from cd.members scd
					   join cte_recommend o on o.recommender = scd.memid)
select o.member, o.recommender, scd.firstname, scd.surname from cte_recommend o
join cd.members scd on scd.memid = o.recommender
where o.member = 22 or o.member = 12
order by o.member asc, o.recommender desc