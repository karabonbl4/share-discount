...
with recursive cte_recommend as(select fst.memid as memid,
					   fst.firstname, fst.surname from cd.members fst
					   where fst.recommendedby = 1
					   union all
					   select scd.memid as memid,
					   scd.firstname, scd.surname from cd.members scd
					   join cte_recommend o on o.memid = scd.recommendedby)
select o.memid, scd.firstname, scd.surname from cte_recommend o
join cd.members scd on scd.memid = o.memid
order by o.memid asc